...
motifs = c("CCGCGG_m-1", "AGAGC_a-2", "CCGAGC_a-3", "CAAAAA_a-5", "GATC_a-1", "GTACAY_a-4", "GACAAT_a-4","GCGCT_m-1", "GGGCC_m-3", "AGGC_a-0", "GACATG_a-3", "GGCGCTRG_m-4","CGGAG_a-3", "CTGCAG_a-4", "GATATC_a-1", "GAGGAG_a-4","GCGATC_a-3", "TTCGAA_a-5")
...
# plots$plt[[1]] <- plots$plt[[1]] + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())

plots$plt[[2]] <- plots$plt[[2]] + theme(legend.position = "bottom") + labs(title = "")
...
mm_a <- gc_df_a %>% 
  ...
  mutate(sample = "AD") %>% 
  left_join(binary_comparison_all %>% filter(sample == "AD") %>% select(contig, bin, sample, is_contamination) %>% distinct(contig, sample, .keep_all = TRUE))
...
ggplot(aes(y = `1_cov.bam`, x = gc * 100)) +
  ...
  coord_cartesian(ylim = c(0,1200), xlim=c(40,70)) +
  labs(
    y = "Coverage",
    x = "GC (%)"
  ) +
