...
plots$plt[[1]] <- plots$plt[[1]] + labs(title = "Simple Fecal") + theme(
...
save(dplt, file = "../RData/fecal_simple_hm_w_include.RData")
...
ggplot(aes(y = `1_cov.bam`, x = gc * 100)) +
  ...
  geom_point(data = mm_bin_mge, aes(shape = mge, size = length), alpha = 0.7, color = "darkblue") +
  labs(
    y = "Coverage",
    x = "GC (%)"
  ) +
