...
make_plot <- function(facet, ratio) {
  data <- contig_methylation_complete %>% 
    ...
    mutate(
      contig = y_html,
      bin_facet = ifelse(bin == "Unbinned", "Unbinned",paste0(bin, ": Comp. ", Completeness, " - Cont. ", Contamination))
    )
  ...
  plot <- plot +
    ...
    geom_text(data = filter(data, is.na(mean)), label = ".", hjust = 0.5, vjust = 0, size = 3) +
    ...
    # guides(color = guide_legend(override.aes = list(alpha = 1, label = c("●"), size = c(2),  fill = c( "black")))) +
    facet_grid(. ~ bin_facet) +
    # scale_x_continuous(breaks = 1:length(BIN_CONSENSUS_MOTIFS)) +
...
plots$plt[[1]] <- plots$plt[[1]] + labs(title = "Simple Fecal") + theme(axis.text.y = element_markdown(vjust = 0.5, hjust = 0, size = 5), axis.ticks.y = element_line()) 
plots$plt[[9]] <- plots$plt[[9]] + theme(
  legend.position = "bottom", 
  axis.text.x = element_markdown(angle = 90, hjust = 1, vjust = 0.5, size = 5), 
  axis.ticks.x = element_line(),
  # legend.text = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 4)
) +
  guides(color = guide_legend(override.aes = list(alpha = 1, label = c("●"), size = c(2),  fill = c( "black"))))
...
labels <- c("250k", "500k", "750k", "1000k")
breaks <- c(250, 500, 750, 1000) * 1000
...
ggplot(aes(y = `1_cov.bam`, x = gc * 100)) +
  geom_point(aes(size = length, color = bin), alpha = 0.6) + 
  ggrepel::geom_label_repel(data = bin_label, aes(label = bin, fill = bin), alpha = 0.7, box.padding = 0.5, size = 3) +
  geom_point(data = mm_bin, aes(shape = mge, color = bin, size = length), alpha = 0.7) +
  labs(
    y = "Coverage",
    x = "GC (%)"
  ) +
  scale_y_log10() +
  scale_size_continuous(name = "Contig length", breaks = breaks, labels = labels) + #, breaks = breaks, labels = labels
  # scale_color_manual(name = "", values = c("#404080",  "#69B3A2",  "#f09205")) +
  scale_color_manual(name = "", values = tab20_colors) +
  scale_fill_manual(name = "", values = tab20_colors) +
  base_plot_theme() +
  theme(
    legend.position = "bottom",
    aspect.ratio = 1,
