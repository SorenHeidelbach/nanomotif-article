...
make_plot <- function(facet, ratio) {
  ...
  plot <- plot +
    ...
    geom_text(data = filter(data, is.na(mean)), label = "·", hjust = 0.5, vjust = 0.4, size = 3) +
...
ggplot(aes(y = `1_cov.bam`, x = gc * 100)) +
  ...
  scale_y_log10() +
  scale_size_continuous(name = "Contig length", breaks = breaks, labels = labels, range = c(0.1, 5)) + #, breaks = breaks, labels = labels
