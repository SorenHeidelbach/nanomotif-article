...
parse_benchmark_paths_to_datatable_no_chunk <- function(root_path, extension) {
  ...
  files <- list.files(path = root_path,pattern = extension,  full.names = TRUE, recursive = TRUE, include.dirs = TRUE)
  #.../{sample}-contig{contig}-nmotif{nmotif}-motif{motif}-type{mod_type}-pos{pos}-cov{coverage}/chunk_{chunk}
  pattern <- ".*/(.+)-contig(.+)-nmotif(\\d+)-motif(.+)-type(\\w+)-pos(\\d+)-cov(\\d+)"
...
ggsave("../figures/microbemod_benchmark_heatmap.png", hm_mm, bg = "white", width = 10, height = 5)
hm_mm
```
...
mm_result[, c("motif", "motif_markdown", "recall", "coverage_factor", "n_motif_factor")][, tool := "MicrobeMod"],
nm_mm_result[, c("motif","motif_markdown", "recall", "coverage_factor", "n_motif_factor")][, tool := "Nanomotif"]
)
combined_benchmark[
...
hm_combined <- ggplot(combined_benchmark) + 
  ...
  geom_tile(aes(fill = recall), color = "black") + 
  scale_fill_steps(limits = c(0, 1), low = "white",high = PLOT_COLORS[["dark_blue"]], na.value = "gray50", n.breaks = 6) +
  facet_grid(tool ~motif_markdown)+
  geom_text(aes(label=paste0(round(recall*100,   digits = 0), "%"))) +
  theme_minimal() +
  theme(
...
# Bin consensus
```{r}
nm_bin_benchmark <- parse_benchmark_paths_to_datatable_no_chunk("/home/bio.aau.dk/lx38ll/dark-science/motif-identification/analysis/microbemod_benchmark/nanomotif_wf", "bin-motifs.tsv")
nm_bin_benchmark[
  , motif := fifelse(
    motif == "GAG.GAAGCT",
    "G[AG].GAAG[CT]",
...
bin_consensus_benchmark <- ggplot(nm_bin_benchmark) + 
  aes(x =n_motif_factor, y = coverage_factor) + 
  geom_tile(aes(fill = identified), color = "black", linewidth =0.6) + 
  facet_wrap(~motif_markdown, ncol = 3) + 
  theme_minimal() +
  theme(
    strip.text = element_markdown()
