...
match_positions_mod <- match_positions - 1 + mod_pos
motif_pileup_1 <- pileup[position %in% match_positions_mod & mod_type == motif_mod_type & strand == "+" & contig == motif_contig][, label := "CAGGAG\nMiddle mean"]
```
...
match_positions <- start(matches)
match_positions_mod <- match_positions - 1 + mod_pos
motif_pileup_2 <- pileup[position %in% match_positions_mod & mod_type == motif_mod_type & strand == "+" & contig == motif_contig][, label := "CTGCAG\nHigh mean"]
```
```{r}
pattern <- "GATC"
mod_pos <- 1
...
match_positions <- start(matches)
match_positions_mod <- match_positions - 1 + mod_pos
motif_pileup_3 <- pileup[position %in% match_positions_mod & mod_type == motif_mod_type & strand == "+" & contig == motif_contig][, label := "GATC\nLow mean"]
```
