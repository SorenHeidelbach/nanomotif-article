...
M.Ruber_MAT <- read_tsv("/projects/
dark_science/nanomotif/nanomotif_article/data/m_ruber/mtase_linker/MTase_assignment_table.tsv")
Zymo_MAT <- read_tsv("/projects/
dark_science/nanomotif/nanomotif_article/data/ZymoHMW/mtase_linker/MTase_assignment_table.tsv")
```
...
M.Ruber_NAT <- read_tsv("/projects/
dark_science/nanomotif/nanomotif_article/data/m_ruber/mtase_linker/nanomotif_assignment_table.tsv")
Zymo_NAT <- read_tsv("/projects/
dark_science/nanomotif/nanomotif_article/data/ZymoHMW/mtase_linker/nanomotif_assignment_table.tsv")
```
...
TMAT <- TMAT %>%
  mutate(`bin name` = recode(`bin name`, !!!TMAT_bin_species_dic)) %>%
  mutate(bin = `bin name`) %>%
  filter(!is.na(bin))

TNAT <- TNAT %>%
  mutate(`bin` = recode(`bin`, !!!TMAT_bin_species_dic)) %>%
  filter(motif_type != "ambiguous" | (motif_type == "ambiguous" & assigned != FALSE)) %>% #filter ambiguous motifs 
...
TMAT_count_df_long <- pivot_longer(TMAT_count_df, cols = c(MTase_count,assigned_count_MTase, motif_count, assigned_count_motif), names_to = "count_type", values_to = "counts") %>%
...
### Loading MTase-Linker tables
```{r}
sfecal_MAT <- read_tsv("/projects/dark_science/nanomotif/nanomotif_article/data/PaPr00000216MP/mtase_linker/MTase_assignment_table.tsv")
sfecal_NAT <- read_tsv("/projects/dark_science/nanomotif/nanomotif_article/data/PaPr00000216MP/mtase_linker/nanomotif_assignment_table.tsv") %>%
...
sfecal_count_df_long <- pivot_longer(sfecal_count_df, cols = c(MTase_count, assigned_count_MTase, motif_count, assigned_count_motif), names_to = "count_type", values_to = "counts") %>%
  ...
  mutate(count_type = factor(count_type, levels = c("MTase_count", "motif_count", "assigned_count_motif"))) %>%
  mutate(bin = sub("^[^\\.]*\\.", "", bin)) %>%
  filter(!(bin %in% c("bin.2.1", "bin.2.2")))
```
