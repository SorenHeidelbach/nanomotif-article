...
load("/home/bio.aau.dk/wy72yf/Projects/nanomotif-article/figures/zymo_and_mono_heatmap.Rdata")
#method <- readPNG("/home/bio.aau.dk/wy72yf/Projects/restriction-modification-annotation/reports/nanomotif_article_temp/nanomotif_method.png", native = TRUE)

method <- readPNG("/home/bio.aau.dk/wy72yf/Projects/nanomotif-article/figures/nanomotif_method (2).png")
method_grob <- rasterGrob(method, interpolate = TRUE)
```
...
mono_and_zymo_heatmap = ggplot(plot_data[mean > 0.2]) +
  ...
  scale_fill_steps(
    ...
    labels = scales::percent, n.breaks = 6, guide = guide_legend(title.position = "top", direction = "horizontal", override.aes = list(alpha = 1))) +
    #guides(fill = guide_legend(title.position = "top", direction = "horizontal", title.hjust = 0.5)) +
  geom_text(data=plot_data[(n_mod + n_nomod - 1)==0], label = ".", hjust=0.5, vjust=0, size=3) +
  ...
  guides(color = guide_legend(direction = "vertical", override.aes = list(alpha = 1, label = c("★", "●"), size = c(2, 1),  fill = c("orange", "black")))) # Adjust this line as needed
...
```{r}
main_theme <- theme(
    text = element_text(family = "sans", size = 0.5),
...
p4 <- hm_combined + main_theme + theme(legend.position = "right", 
        legend.margin = margin(t = 0, r = -3, b = 0, l = -7, unit = "pt"))
...
```{r}
png("/home/bio.aau.dk/wy72yf/Projects/nanomotif-article/main_figures/figure_1.png", res = 600, height = 14, width = 18, units = "cm")
...
layout3 <- grid.layout(nrow = 2, ncol = 2,
                      widths = unit(c(9, 11), "null"),
                      heights = unit(c(18, 12), "null"), just = "left")
...
# For p1 in the top left area
pushViewport(viewport(layout.pos.row = 1, layout.pos.col = 1))
grid.draw(method_grob)
grid.text("a", x = unit(0, "npc") + unit(4, "mm") , y = unit(1, "npc") - unit(2.5, "mm"), gp = gpar(fontfamily = "sans", fontface = "bold"))
upViewport()
