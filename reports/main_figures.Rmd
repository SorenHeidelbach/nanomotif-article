...
### Loading plot for figure panel 1
```{r}
...
mono_and_zymo_heatmap = ggplot(plot_data[mean > 0.2]) +
  ...
  scale_color_manual(name = NULL, values = c("No motif observations" = "black", "Assigned MTase" = "orange")) +
  guides(color = guide_legend(direction = "vertical", override.aes = list(alpha = 1, label = c("+", "."), size = c(2, 3),  fill = c("orange", "black")))) # Adjust this line as needed
...
p3 <- mono_and_zymo_heatmap + main_theme + theme(legend.key.size = unit(0.2, "cm"),
                                                 ...
                                                 axis.text.y = element_markdown(size=6, family = "sans"), # Rotate x-axis labels to 90 degrees
                                                  axis.text.x = element_markdown(size = 6, family = "sans")
)
...
```{r}
pdf("/home/bio.aau.dk/wy72yf/Projects/nanomotif-article/main_figures/figure_1.pdf", height = 5.51181102, width = 7.08661417, family = "sans")
