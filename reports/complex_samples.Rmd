...
load_complex_sample <- function(
  ...
  motifs_full <- motifs_full[!is.na(bin)]
...
ad <- load_complex_sample(
  paste0("../data/", sample, "/nanomotif/motifs.tsv"),
  paste0("../data/", sample, "/nanomotif/motifs-scored.tsv"),
  paste0("../data/", sample, "/nanomotif/bin-motifs.tsv"),
  paste0("../data/", sample, "/mmlong2_lite/tmp/binning/contig_bin.tsv"),
  paste0("../data/", sample, "/mmlong2_lite/results/mmlong2_lite_bins.tsv")
)
ad_hq_bin = ad[["bin_info"]][bin_quality == "HQ"]$bin
sample_stats <- data.table(
  n_bin = nrow(ad[["bin_info"]]),
  n_hq = nrow(ad[["bin_info"]][bin_quality == "HQ"]),
  n_mq = nrow(ad[["bin_info"]][bin_quality == "MQ"]),
  n_bin_with_motif = length(unique(ad[["bin_motifs"]]$bin)),
  n_hq_with_motif = sum(unique(ad[["bin_motifs"]]$bin) %in% ad_hq_bin),
  n_motif = nrow(ad[["bin_motifs"]][mean > 0.5]),
  n_motif_hq = nrow(ad[["bin_motifs"]][bin %in% ad_hq_bin][mean > 0.5]),
  sample = "anaerobic_digestor"
)
sample_stats
```
...
```{r}
sample = "fecal"
fecal_complex <- load_complex_sample(
  paste0("../data/", sample, "/nanomotif/motifs.tsv"),
  paste0("../data/", sample, "/nanomotif/motifs-scored.tsv"),
  paste0("../data/", sample, "/nanomotif/bin-motifs.tsv"),
  paste0("../data/", sample, "/mmlong2_lite/tmp/binning/contig_bin.tsv"),
...
fecal_hq <- fecal_complex[["bin_info"]][bin_quality == "HQ"]$bin
sample_stats <- rbind(sample_stats, data.table(
  n_bin = nrow(fecal_complex[["bin_info"]]),
  n_hq = nrow(fecal_complex[["bin_info"]][bin_quality == "HQ"]),
  n_mq = nrow(fecal_complex[["bin_info"]][bin_quality == "MQ"]),
  n_bin_with_motif = length(unique(fecal_complex[["bin_motifs"]]$bin)),
  n_hq_with_motif = sum(unique(fecal_complex[["bin_motifs"]]$bin) %in% fecal_hq),
  n_motif = nrow(fecal_complex[["bin_motifs"]][mean > 0.5]),
  n_motif_hq = nrow(fecal_complex[["bin_motifs"]][bin %in% fecal_hq][mean > 0.5]),
  sample = "fecal_complex"
))
sample_stats
```

```{r}
simple_fecal <- load_complex_sample(
  "../data/PaPr00000216MP_nm_0.1.19/nanomotif/motifs.tsv",
  "../data/PaPr00000216MP_nm_0.1.19/nanomotif/motifs-scored.tsv",
  "../data/PaPr00000216MP_nm_0.1.19/nanomotif/bin-motifs.tsv",
  "../data/PaPr00000216MP_nm_0.1.19/bins.tsv",
  "../data/PaPr00000216MP_nm_0.1.19/NP-PaPr00000216MP_bins.tsv"
)
fecal_simple_hq <- simple_fecal[["bin_info"]][bin_quality == "HQ"]$bin
sample_stats <-  rbind(sample_stats, data.table(
  n_bin = nrow(simple_fecal[["bin_info"]]),
  n_hq = nrow(simple_fecal[["bin_info"]][bin_quality == "HQ"]),
  n_mq = nrow(simple_fecal[["bin_info"]][bin_quality == "MQ"]),
  n_bin_with_motif = length(unique(simple_fecal[["bin_motifs"]]$bin)),
  n_hq_with_motif = sum(unique(simple_fecal[["bin_motifs"]]$bin) %in% fecal_simple_hq),
  n_motif = nrow(simple_fecal[["bin_motifs"]][mean > 0.5]),
  n_motif_hq = nrow(simple_fecal[["bin_motifs"]][bin %in% fecal_simple_hq][mean > 0.5]),
  sample = "simple_fecal"
))
sample_stats
```
```{r}
soil <- load_complex_sample(
  "../data/soil_mfd02199/nanomotif/motifs.tsv",
  "../data/soil_mfd02199/nanomotif/motifs-scored.tsv",
  "../data/soil_mfd02199/nanomotif/bin-motifs.tsv",
  "../data/soil_mfd02199/mmlong2_lite/tmp/binning/contig_bin.tsv",
  "../data/soil_mfd02199/mmlong2_lite/results/mmlong2_lite_bins.tsv"
)
soil_hq <- soil[["bin_info"]][bin_quality == "HQ"]$bin
sample_stats <-  rbind(sample_stats, data.table(
  n_bin = nrow(soil[["bin_info"]]),
  n_hq = nrow(soil[["bin_info"]][bin_quality == "HQ"]),
  n_mq = nrow(soil[["bin_info"]][bin_quality == "MQ"]),
  n_bin_with_motif = length(unique(soil[["bin_motifs"]]$bin)),
  n_hq_with_motif = sum(unique(soil[["bin_motifs"]]$bin) %in% fecal_simple_hq),
  n_motif = nrow(soil[["bin_motifs"]][mean > 0.5]),
  n_motif_hq = nrow(soil[["bin_motifs"]][bin %in% fecal_simple_hq][mean > 0.5]),
  sample = "soil"
))
sample_stats
```
...
n_mq <- nrow(simple_fecal[["bin_info"]][bin_quality == "MQ"])
n_motif <- nrow(simple_fecal[["bin_motifs"]][mean > 0.5])
motif_pr_bin <- n_motif / (n_hq + n_mq)
```


```{r}
plot_data <- soil[["motifs_full"]][bin %in% soil_hq][
...
bin_contigs_heatmap <- ggplot(plot_data) +
  ...
  scale_fill_gradient2(guide = "none", limits = c(0, 1), low = "white", high = PLOT_COLORS[[1]], na.value = "white") +
...
width_hm <- 16
ggsave(
  paste0("../figures/", sample, "_bin_facet_heatmap2.png"),
  bin_contigs_heatmap,
  height = height_hm,
  width = width_hm,
  dpi = "retina",
